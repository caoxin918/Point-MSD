...
class Point2Vec(pl.LightningModule):
    def __init__(
        ...
        from point2vec.modules.point_mamba import Mamba, MambaDecoder, MixerModel
        self.student = MixerModel(d_model=encoder_dim,
        ...
        self.increase_dim = nn.Sequential(nn.Conv1d(encoder_dim, 3 * tokenizer_group_size, 1))
    ...
    def forward(
        ...
        # embeddings, neighborhood, centers = self.tokenizer(inputs)  # (B, T, C), (B, T, 3)
        ...
        if self.hparams.decoder:  # type: ignore
            ...
            output_embeddings, _ = self.student(
                visible_embeddings, visible_pos
            )  # (B, T, C).last_hidden_state
        ...
        rebuild_points = self.increase_dim(predictions1.transpose(1, 2)).transpose(1, 2).reshape(B * M, -1,
                                                                                                 3)  # BM Gs 6

        gt_points = neighborhood[mask].reshape(B * M, -1, 3)

        predictions_g = torch.mean(predictions1, dim=1).reshape([B, -1])
        targets_g = torch.mean(targets.reshape([B, -1, C]), dim=1).reshape([B, -1])

        return predictions, targets, rebuild_points.float(), gt_points.float(), predictions_g, targets_g

    def _perform_step(self, inputs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        # inputs: (B, N, 3)
        tokens, neighborhood, centers = self.tokenizer(inputs)  # (B, T, C), (B, T, 3)
        mask = self.masking(centers)  # (B, T)
        return self.forward(tokens, neighborhood, centers, mask)

    def training_step(self, batch, batch_idx: int) -> torch.Tensor:
        # inputs: (B, N, 3)
        points, _ = batch
        points = self.train_transformations(points)
        x, y, rebuild_points, gt_points, xg, yg = self._perform_step(points)
        loss = self.loss_func(x, y)
        # cd_loss = self.cdl2_loss(rebuild_points, gt_points)
        loss_g = self.loss_func(xg, yg)
        self.log("train_loss", loss, on_epoch=True)
        # self.log("train_loss_cd", cd_loss, on_epoch=True)
    ...
    def generate_targets(
        ...
        # assert self.teacher.ema_model is not None  # always false # mamba not
        # self.teacher.ema_model.eval() # mamba not
        ...
        output, all_output = self.teacher(
            tokens,
            pos,
        ...
        if d2v_target_layer_part == "ffn":
            ...
            # assert output.hidden_states is not None # mamba not
            # target_layers = output.hidden_states
            target_layers = all_output
        else:
            raise ValueError()
        target_layers = [
