...
class ModelNet40Ply2048(Dataset):
    ...
    def __getitem__(self, item):
        ...
        # add noise
        # noise_points = self.noise_Gaussian(points, 1000)
        label = self.labels[item]
        return points, label

    def noise_Gaussian(self, points, num):
        _, C = points.shape
        noise = np.random.normal(0, 1, [num, C]).astype(np.float32)
        out = np.vstack([points, noise])
        return out
